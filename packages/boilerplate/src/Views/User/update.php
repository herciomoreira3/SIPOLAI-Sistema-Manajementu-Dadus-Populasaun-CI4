        ...
        <div class="card card-outline card-info">
            ...
            <div class="card-body">
                <form action="<?= base_url('admin/user/manage/'.$user['id']) ?>/update" method="post" class="form-horizontal">
                    ...
                    <?php $hasPwError = session('error.password') || session('error.pass_confirm'); ?>
                    
                    <div class="form-group row" id="pw-reset-row" style="<?= $hasPwError ? 'display: none;' : '' ?>">
                        <label class="col-sm-2 col-form-label"></label>
                        <div class="col-sm-8">
                            <button type="button" id="btn-pw-reset" class="btn btn-warning btn-sm" style="border-radius: 8px; font-weight: 600;">
                                <i class="fas fa-key mr-1"></i> Ganti Password Foun
                            </button>
                        </div>
                    </div>

                    <div class="form-group row" id="pw-row-1" style="<?= $hasPwError ? '' : 'display: none;' ?>">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                ...
                                <input type="password" name="password" id="input-password" class="form-control <?= session('error.password') ? 'is-invalid' : '' ?>" placeholder="<?=lang('Auth.password')?>" autocomplete="off" <?= $hasPwError ? '' : 'disabled' ?>>
                    ...
                    <div class="form-group row" id="pw-row-2" style="<?= $hasPwError ? '' : 'display: none;' ?>">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                ...
                                <input type="password" name="pass_confirm" id="input-pass-confirm" class="form-control <?= session('error.pass_confirm') ? 'is-invalid' : '' ?>" placeholder="<?=lang('Auth.repeatPassword')?>" autocomplete="off" <?= $hasPwError ? '' : 'disabled' ?>>
...
<?= $this->section('js') ?>
  ...
  // Reset Password UI toggle
  $('#btn-pw-reset').on('click', function() {
      $('#pw-reset-row').hide();
      $('#pw-row-1').show();
      $('#pw-row-2').show();
      $('#input-password').prop('disabled', false).focus();
      $('#input-pass-confirm').prop('disabled', false);
  });
  </script>
<?= $this->endSection() ?>
