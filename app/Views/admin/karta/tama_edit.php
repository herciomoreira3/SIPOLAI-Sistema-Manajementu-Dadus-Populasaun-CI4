    ...
    <div class="col-md-8 mx-auto">
        <div class="card card-premium card-outline card-primary">
            <div class="card-header bg-transparent border-0 pt-4 px-4">
                <h3 class="card-title font-weight-bold text-secondary"><i class="fas fa-edit mr-2"></i> Hadia Karta Tama</h3>
            </div>
            <form action="<?= base_url('admin/karta-tama/' . $karta['id_karta_tama']) ?>" method="POST" class="px-4 pb-4 pt-2">
                <?= csrf_field() ?>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="emitente" id="emitente" class="form-control <?= session('errors.emitente') ? 'is-invalid' : '' ?>" value="<?= old('emitente') ?? esc($karta['emitente']) ?>" placeholder="Fila emitente/husi..." required>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="date" name="data_tama" id="data_tama" class="form-control <?= session('errors.data_tama') ? 'is-invalid' : '' ?>" value="<?= old('data_tama') ?? esc($karta['data_tama']) ?>" required>
