    ...
    <div class="col-md-8 mx-auto">
        <div class="card card-premium card-outline card-primary">
            <div class="card-header bg-transparent border-0 pt-4 px-4">
                <h3 class="card-title font-weight-bold text-secondary"><i class="fas fa-plus mr-2"></i> Rejista Manual Membru Struktura</h3>
            </div>
            <form action="<?= base_url('admin/estrutura') ?>" method="POST" class="px-4 pb-4 pt-2">
                ...
                <div class="form-group">
                    ...
                    <select name="id_populasaun" id="id_populasaun" class="form-select select2" style="width: 100%;">
                        ...
                        <?php foreach ($populasaun as $pop) : ?>
                            <option value="<?= $pop['id_populasaun'] ?>" data-aldeia-id="<?= $pop['id_aldeia'] ?>" <?= old('id_populasaun') == $pop['id_populasaun'] ? 'selected' : '' ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="naran_membru" id="naran_membru" class="form-control <?= session('errors.naran_membru') ? 'is-invalid' : '' ?>" value="<?= old('naran_membru') ?>" placeholder="Fila naran kompletu..." required>
                ...
                <div class="form-group">
                    ...
                    <select name="kargu" id="kargu" class="form-select <?= session('errors.kargu') ? 'is-invalid' : '' ?>" required>
                        <option value="">-- Hili Kargu --</option>
                        <?php foreach ($kargus as $kg) : ?>
                            <option value="<?= esc($kg['naran_kargu']) ?>" <?= old('kargu') == $kg['naran_kargu'] ? 'selected' : '' ?>><?= esc($kg['naran_kargu']) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group" id="aldeia-wrapper">
                    ...
                    <select name="id_aldeia" id="id_aldeia" class="form-select">
                        ...
                        <?php foreach ($aldeias as $ald) : ?>
                            <option value="<?= $ald['id_aldeia'] ?>" <?= old('id_aldeia') == $ald['id_aldeia'] ? 'selected' : '' ?>><?= esc($ald['naran_aldeia']) ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="periodo_hahula" id="periodo_hahula" class="form-control <?= session('errors.periodo_hahula') ? 'is-invalid' : '' ?>" value="<?= old('periodo_hahula') ?>" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="periodo_remata" id="periodo_remata" class="form-control" value="<?= old('periodo_remata') ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status_kargu" id="status_kargu" class="form-select">
                        <option value="Ativu" <?= old('status_kargu') == 'Ativu' ? 'selected' : '' ?>>Ativu</option>
                        <option value="Inativu" <?= old('status_kargu') == 'Inativu' ? 'selected' : '' ?>>Inativu</option>
                    </select>
                </div>

                <div class="d-flex justify-content-between mt-4">
                    <a href="<?= base_url('admin/estrutura') ?>" class="btn btn-outline-secondary btn-rounded">Fila</a>
                    <button type="submit" class="btn btn-primary btn-rounded">Mete Dadus</button>
                </div>
            </form>
        </div>
    </div>
