            ...
            <div class="card-header bg-transparent border-0 pt-4 px-4">
                ...
                <?php else : ?>
                    <div class="text-center">
                        ...
                        <h4 class="text-secondary font-weight-bold mb-3">Relatoriu Kbiit Laek Active</h4>
                    </div>
                <?php endif; ?>
                    ...
                    <table class="table table-bordered table-striped align-middle">
                        ...
                        <tbody>
                            <?php if (empty($membros)) : ?>
                                <tr>
                                    <td colspan="7" class="text-center text-muted">Laiha dadus kbiit laek ativu.</td>
                                </tr>
                            <?php else : ?>
                                <?php $no = 1; foreach ($membros as $m) : ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td class="font-weight-bold text-secondary"><?= esc($m['nik']) ?></td>
                                        <td class="font-weight-bold text-success"><?= esc($m['no_kbiit_laek']) ?></td>
                                        <td class="font-weight-bold text-secondary"><?= esc($m['naran_kompletu']) ?></td>
                                        <td><?= esc($m['jeneru']) ?></td>
                                        <td><?= esc($m['naran_aldeia']) ?></td>
                                        <td><?= esc($m['naran_profisaun']) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
